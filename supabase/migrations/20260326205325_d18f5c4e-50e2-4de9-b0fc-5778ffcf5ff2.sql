...
-- Fix: get_next_sales_user() has UPDATE without WHERE clause on overflow_round_robin_state
-- This causes "UPDATE requires a WHERE clause" error when all primary agents are at cap
...
BEGIN
  ...
  -- ALL PRIMARY AGENTS AT CAP: use overflow recipients
  ...
  -- FIX: Update overflow round-robin state WITH a WHERE clause
  IF v_next_user_id IS NOT NULL THEN
