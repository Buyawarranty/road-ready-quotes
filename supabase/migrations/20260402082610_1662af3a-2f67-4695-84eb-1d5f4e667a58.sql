...
-- 1) REVERT: Restore the trigger so ALL leads (including google_ad) go through round-robin
...
-- 2) NEW TRIGGER: When a Google Ad lead is converted, auto-reassign to Website
-- This fires when status changes TO 'converted' on a google_ad lead
...
BEGIN
  ...
  IF NEW.status = 'converted' AND NEW.lead_source = 'google_ad' THEN
    -- Force assignment to NULL (Website) regardless of who converted it
    NEW.assigned_to := NULL;
    NEW.assigned_at := NULL;
  END IF;

  -- Also prevent agents from reassigning converted Google Ad leads
  -- If the lead is already converted + google_ad, and someone tries to change assigned_to
  IF OLD.status = 'converted' AND OLD.lead_source = 'google_ad' 
     AND NEW.assigned_to IS DISTINCT FROM OLD.assigned_to THEN
    -- Keep it as Website (NULL)
    NEW.assigned_to := NULL;
    NEW.assigned_at := NULL;
  END IF;
...
DROP TRIGGER IF EXISTS trg_auto_reassign_google_ad_conversion ON public.sales_leads;
CREATE TRIGGER trg_auto_reassign_google_ad_conversion
  BEFORE UPDATE ON public.sales_leads
  FOR EACH ROW
  EXECUTE FUNCTION public.auto_reassign_google_ad_conversion();

-- 3) Fix Christopher Lyons: he was a Google Ad conversion, reassign to Website
UPDATE sales_leads
SET assigned_to = NULL, assigned_at = NULL, status = 'converted', updated_at = now()
WHERE lower(btrim(email)) = 'chrislyons1989@icloud.com'
  AND lead_source = 'google_ad';

UPDATE customers
SET assigned_to = 'e39499b8-f88c-4963-9f0d-63e1addb3025'
WHERE lower(btrim(email)) = 'chrislyons1989@icloud.com'
  AND assigned_to != 'e39499b8-f88c-4963-9f0d-63e1addb3025';
