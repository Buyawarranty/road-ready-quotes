...
-- Fix the trigger: do NOT use email local part as first_name fallback.
-- If there's no real name, leave first_name as NULL.
...
BEGIN
  ...
  -- Only extract name if full_name exists AND is NOT an email address
  ...
  ELSE
    -- No real name available — leave as NULL (do NOT use email local part)
    v_first_name := NULL;
    v_last_name := NULL;
  END IF;
  ...
  -- ============================================================
  -- 4) TERMINAL GUARD: If this person was EVER marked as lost,
  --    fake, or converted — do NOT create a new lead.
  -- ============================================================
  IF length(v_clean_phone) >= 10 THEN
  ...
  -- ============================================================
  -- 5) NEW LEAD: No match found — insert new lead
  -- ============================================================
