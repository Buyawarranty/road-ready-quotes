...
-- Fix: Google Ad leads should NOT be assigned to agents via round-robin.
-- They should remain as "Website" (support account or NULL).
-- Only website and social_ad leads get auto-assigned to agents.
...
BEGIN
  ...
  -- IMPORTANT: Google Ad leads are NOT assigned to agents - they stay as Website
  BEGIN
    IF v_derived_source = 'google_ad' THEN
      -- Google Ad leads: do NOT assign to any agent, leave as Website
...
-- Fix Christopher Lyons: reassign Google Ad lead back to Website (NULL = unassigned/Website)
UPDATE sales_leads
...
-- Also fix the customer record back to the website support account
UPDATE customers
