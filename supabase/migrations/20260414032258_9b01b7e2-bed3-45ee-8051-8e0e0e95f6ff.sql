...
-- 1) Clean up existing duplicate leads (same email + same created_at = race condition duplicates)
-- Keep the one with the most activity, delete the rest
WITH ranked_dupes AS (
  SELECT 
    ...
    created_at,
    ROW_NUMBER() OVER (
      PARTITION BY email, created_at
      ORDER BY 
        CASE WHEN call_count > 0 OR notes IS NOT NULL THEN 0 ELSE 1 END,
        CASE WHEN status NOT IN ('new') THEN 0 ELSE 1 END,
...
-- 2) Update the trigger with advisory locking to prevent race condition duplicates
...
BEGIN
  ...
  -- === ADVISORY LOCK on email hash to prevent race condition duplicates ===
