...
-- Fix 1: recover_orphaned_leads - move get_next_sales_user() inside the INSERT exception block
-- so failed inserts don't ghost-advance the round-robin pointer. Also fix lead_status enum cast.
...
BEGIN
  ...
  LOOP
    ...
    -- CRITICAL FIX: get_next_sales_user() and INSERT in the SAME exception block.
    -- If INSERT fails, the round-robin advance rolls back with the subtransaction.
    BEGIN
...
-- Fix 2: auto_create_lead_from_abandoned_cart - same fix: move get_next_sales_user()
-- inside the INSERT exception block so failed inserts don't ghost-advance round-robin.
...
BEGIN
  ...
  -- 5+6) CRITICAL FIX: get_next_sales_user() and INSERT in the SAME exception block.
  -- If INSERT fails, the round-robin advance rolls back with the subtransaction.
  BEGIN
    v_assigned_agent := public.get_next_sales_user();
    ...
    INSERT INTO public.sales_leads (
      ...
      'website', 'new'::lead_status, 'medium',
