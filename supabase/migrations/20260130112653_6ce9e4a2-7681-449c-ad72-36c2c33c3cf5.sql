-- Fix the claim_lead_for_agent function to assign the correct admin_users.id
-- instead of the auth user_id
...
BEGIN
    ...
    -- FIXED: Assign the lead using admin_users.id (p_agent_id) directly
    -- NOT the auth user_id. The assigned_to column stores admin_users.id
    UPDATE sales_leads
