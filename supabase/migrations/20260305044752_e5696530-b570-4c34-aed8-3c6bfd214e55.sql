...
-- CRITICAL FIX: The auto_create_lead_from_abandoned_cart trigger was resetting
-- status = 'new' on dedup matches, overriding agent-set statuses.
-- Also phone was being overwritten by empty strings from later cart steps.
-- 
-- Changes:
-- 1. REMOVE status = 'new' from phone dedup and email dedup paths
-- 2. Protect phone from empty string overwrite using NULLIF
-- 3. Preserve assigned_to, notes, and all agent-set fields on dedup
  ...
  IF length(v_clean_phone) >= 10 THEN
    ...
    IF v_dedup_lead_id IS NOT NULL THEN
      ...
      SET
        ...
        updated_at = now()
        -- REMOVED: status = 'new' — agent-set status is now preserved
