...
-- 1) Update the trigger to allow paid sources through the terminal guard
...
BEGIN
  ...
  -- 2) PHONE DEDUP — match within 7 days OR any lead with activity (assigned/called/noted)
  ...
  IF length(v_clean_phone) >= 10 THEN
    ...
    IF v_dedup_lead_id IS NOT NULL THEN
      ...
      SET
        ...
        updated_at = now()
        -- NOTE: assigned_to is intentionally NOT updated to protect existing assignment
      WHERE id = v_dedup_lead_id;
      RETURN NEW;
    END IF;
  END IF;

  -- 3) EMAIL DEDUP — match within 7 days OR any lead with activity
  SELECT id INTO v_dedup_lead_id
  FROM public.sales_leads
  ...
  -- 4) TERMINAL GUARD — only block ORGANIC (website) repeat leads; paid sources always get a new lead
  IF length(v_clean_phone) >= 10 THEN
    ...
    IF v_terminal_lead_id IS NOT NULL THEN
      ...
      -- Only block organic leads; paid sources proceed to create a new lead
  ...
  -- 6) INSERT — ALL leads get round-robin assigned (including google_ad)
  BEGIN
...
-- 2) Backfill: Create leads from historically blocked paid sources
DO $$
DECLARE
  rec RECORD;
  v_agent UUID;
...
BEGIN
  FOR rec IN
    SELECT sel.id AS log_id, sel.event_data, ac.*
    FROM system_event_logs sel
    JOIN abandoned_carts ac ON ac.id = (sel.event_data->>'cart_id')::uuid
    WHERE sel.event_type = 'lead_blocked_by_terminal'
      AND sel.event_data->>'lead_source' IN ('google_ad', 'social_ad')
      AND NOT EXISTS (
  ...
  LOOP
    v_source := (rec.event_data->>'lead_source')::lead_source;
    v_agent := public.get_next_sales_user();
    ...
    INSERT INTO sales_leads (
      ...
      NULLIF(split_part(COALESCE(rec.full_name, ''), ' ', 1), ''),
      NULLIF(btrim(substring(COALESCE(rec.full_name, '') from char_length(split_part(COALESCE(rec.full_name, ''), ' ', 1)) + 1)), ''),
      lower(btrim(rec.email)),
      NULLIF(btrim(rec.phone), ''),
      v_source, 'new'::lead_status, 'medium',
      NULLIF(btrim(rec.vehicle_reg), ''),
      NULLIF(btrim(rec.vehicle_make), ''),
      NULLIF(btrim(rec.vehicle_model), ''),
      NULLIF(btrim(rec.vehicle_year), ''),
      NULLIF(btrim(rec.vehicle_type), ''),
      NULLIF(btrim(rec.mileage), ''),
      rec.plan_name, rec.total_price, rec.id,
      rec.created_at, now(), now(),
      0, v_agent, CASE WHEN v_agent IS NOT NULL THEN now() ELSE NULL END
    );
...
-- 3) Re-derive misattributed lead sources where cart has gclid/fbclid but lead says 'website'
UPDATE sales_leads sl
SET lead_source = CASE
  WHEN ac.cart_metadata->>'gclid' IS NOT NULL AND ac.cart_metadata->>'gclid' != '' THEN 'google_ad'::lead_source
  WHEN ac.cart_metadata->>'fbclid' IS NOT NULL AND ac.cart_metadata->>'fbclid' != '' THEN 'social_ad'::lead_source
  ELSE sl.lead_source
END,
updated_at = now()
...
WHERE sl.abandoned_cart_id = ac.id
  ...
  AND (
    (ac.cart_metadata->>'gclid' IS NOT NULL AND ac.cart_metadata->>'gclid' != '')
    OR (ac.cart_metadata->>'fbclid' IS NOT NULL AND ac.cart_metadata->>'fbclid' != '')
  );
