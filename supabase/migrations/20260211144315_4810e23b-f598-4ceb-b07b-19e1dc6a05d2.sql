...
-- Drop and recreate the sync function with correct column references
...
BEGIN
  ...
  -- Sync from sales_leads (uses first_name + last_name, NOT full_name)
  ...
  -- Sync from abandoned_carts (uses full_name directly)
  INSERT INTO marketing_audience (lead_id, reg_plate, mileage, email, phone, full_name, source, source_type, lead_status, synced_at)
  SELECT 
    ac.id,
    ac.vehicle_reg,
