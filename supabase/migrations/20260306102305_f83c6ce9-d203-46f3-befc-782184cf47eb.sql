...
-- Fix round-robin: auto-assign new leads in the trigger and increment assigned_today counter
...
BEGIN
  ...
  -- 5) ROUND-ROBIN ASSIGNMENT: Get next agent before inserting
  v_assigned_agent := public.get_next_sales_user();

  -- 6) NEW LEAD with auto-assignment
