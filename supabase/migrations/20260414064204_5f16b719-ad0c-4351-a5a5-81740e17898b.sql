...
-- 1) Update the trigger to check ALL carts for this email when deriving source
...
BEGIN
  ...
  IF v_derived_source = 'website' THEN
    -- Check for gclid in any cart for this email
    IF EXISTS (
    ...
    -- Check for fbclid or facebook utm_source in any cart for this email
    ELSIF EXISTS (
      SELECT 1 FROM public.abandoned_carts
      WHERE lower(btrim(email)) = v_clean_email
        AND cart_metadata IS NOT NULL
        AND (
          ...
          OR cart_metadata->>'utm_source' IN ('facebook', 'fb', 'ig')
        )
      LIMIT 1
    ) THEN
...
-- 2) Backfill: Fix leads misattributed as 'website' that should be 'google_ad'
UPDATE sales_leads sl
SET lead_source = 'google_ad'
WHERE sl.lead_source = 'website'
  AND EXISTS (
    SELECT 1 FROM abandoned_carts ac
    WHERE lower(btrim(ac.email)) = lower(btrim(sl.email))
      AND ac.cart_metadata::text LIKE '%gclid%'
  );

-- 3) Backfill: Fix leads misattributed as 'website' that should be 'social_ad' (no gclid overlap)
UPDATE sales_leads sl
SET lead_source = 'social_ad'
WHERE sl.lead_source = 'website'
  AND NOT EXISTS (
    SELECT 1 FROM abandoned_carts ac
    WHERE lower(btrim(ac.email)) = lower(btrim(sl.email))
  ...
  AND EXISTS (
    ...
    WHERE lower(btrim(ac.email)) = lower(btrim(sl.email))
      ...
      AND (
        ac.cart_metadata::text LIKE '%fbclid%'
        OR ac.cart_metadata->>'utm_source' IN ('facebook', 'fb', 'ig')
      )
  );
