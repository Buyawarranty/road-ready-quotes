...
-- Fix overflow round-robin: use sort_order comparison instead of UUID id comparison
-- The bug: comparing id > v_last_overflow_id uses UUID lexicographic order,
-- which doesn't match the intended sort_order sequence, causing one agent
-- to receive all overflow leads.
