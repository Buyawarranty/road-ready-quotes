...
BEGIN
  ...
  -- TRUNCATE and rebuild to avoid any duplicate issues on re-sync
  TRUNCATE marketing_audience;

  -- Insert all unique contacts from sales_leads and abandoned_carts combined
  -- Use a single query with UNION ALL then deduplicate with DISTINCT ON
  INSERT INTO marketing_audience (lead_id, reg_plate, mileage, email, phone, full_name, source, source_type, lead_status, synced_at)
  SELECT sub.lead_id, sub.reg_plate, sub.mileage, sub.email, sub.phone, sub.full_name, sub.source, sub.source_type, sub.lead_status, now()
  FROM (
