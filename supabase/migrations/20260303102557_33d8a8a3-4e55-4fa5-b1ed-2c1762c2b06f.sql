...
BEGIN
  ...
  -- 1) Check for existing lead linked to this cart (idempotency)
  ...
  -- 2) Phone-based dedup: if a lead with the same phone exists in the last 7 days,
  --    update it and bump it to the top + mark as resubmission
  ...
  IF length(v_clean_phone) >= 10 THEN
    ...
    IF v_phone_match_id IS NOT NULL THEN
      UPDATE public.sales_leads
      SET
        email = lower(NEW.email),
        first_name = COALESCE(NULLIF(v_first_name, ''), first_name),
        ...
        -- Bump to top and mark as resubmission
        last_activity_date = now(),
        resubmission_count = COALESCE(resubmission_count, 0) + 1,
        last_resubmitted_at = now(),
        status = 'new',
        updated_at = now()
      WHERE id = v_phone_match_id;
      RETURN NEW;
    END IF;
  END IF;

  -- 3) New lead: insert. Round robin assignment happens in trg_auto_assign_lead.
