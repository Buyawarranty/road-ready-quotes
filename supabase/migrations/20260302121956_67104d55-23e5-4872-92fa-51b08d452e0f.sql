-- Align legacy selector with strict default sequence: James(1) -> Isobel(2) -> Ash(3), then overflow.
...
BEGIN
  -- Keep caps current before selecting
  PERFORM public.reset_daily_caps();

  -- Primary strict round-robin based on sort_order across active sales + sales_lead users
  ...
  ORDER BY adc.sort_order ASC NULLS LAST, adc.last_assigned_at ASC NULLS FIRST
  LIMIT 1;
  ...
  -- Persist pointer when a primary agent is selected
  ...
  -- All primary agents are at cap: use overflow recipients in their own sequence
