...
serve(async (req) => {
  ...
  try {
    ...
    const brand = resolveBrand(req, { brand: customerData?.brand });
    const redirectUrl = url.searchParams.get('redirect') || `${brand.siteUrl}/thank-you`;
    ...
    const errorRedirect = req.headers.get("origin") || resolveBrand(req).siteUrl;
    return new Response(null, {
      status: 302,
      headers: {
        ...corsHeaders,
